...
# run app
java -jar target/java-backend-parson-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/java-backend-parson:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/java-backend-parson -n dev

# get external endpoint
kubectl get svc java-backend-parson-svc -n dev
```
...
```bash
kubectl port-forward svc/java-backend-parson-svc 8080:80 -n dev
...
kubectl rollout restart deploy/java-backend-parson -n dev
kubectl rollout status deploy/java-backend-parson -n dev
```
...
info.app.name=javabackendparson
...
├─ src/
│  ├─ main/
│  │  ├─ java/com/harness/javabackendparson/
│  │  │  ├─ App.java
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/javabackendparson/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
